    ...
    <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-md p-6 space-y-4">
        <div className="flex items-start justify-between">
          <div>
            <h3 className="text-base font-semibold text-slate-950 dark:text-white">Force Check-In</h3>
            <p className="text-sm text-slate-600 mt-0.5">{lecturer.name} · {lecturer.scheduledClass}</p>
          </div>
          <button onClick={onCancel} className="p-1.5 rounded-lg text-slate-600 dark:text-slate-400 hover:text-gray-600 cursor-pointer">
            <X size={16} />
          </button>
        </div>
        ...
        <div>
          <label className="block text-xs font-semibold text-slate-600 dark:text-slate-400 mb-1.5">
            ...
            <span className="ml-auto float-right text-slate-600 dark:text-slate-400 font-normal">{reason.length}/{MAX}</span>
          </label>
          <textarea
            value={reason}
            ...
            rows={2}
            className="w-full px-3 py-2 rounded-xl text-sm bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 text-slate-950 dark:text-white placeholder:text-slate-500 focus:outline-none focus:ring-2 focus:ring-blue-500/40 resize-none"
          />
        </div>
        ...
        <div className="flex gap-2 pt-1">
          <button
            onClick={onCancel}
            className="flex-1 py-2 rounded-xl text-sm font-medium text-slate-600 dark:text-slate-400 bg-gray-100 dark:bg-white/5 hover:bg-gray-200 dark:hover:bg-white/10 transition-colors cursor-pointer"
          >
            Cancel
          </button>
          <button
...
export function LecturerPresencePage() {
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-2xl font-bold text-slate-950 dark:text-white">Lecturer Presence</h1>
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="glass-card p-4 text-center">
          ...
          <p className="text-xs text-slate-600 mt-0.5">🟢 On Time</p>
        </div>
        <div className="glass-card p-4 text-center">
          <p className="text-2xl font-bold text-amber-600 dark:text-amber-400">{late}</p>
          <p className="text-xs text-slate-600 mt-0.5">🟡 Late</p>
        </div>
        <div className="glass-card p-4 text-center">
          <p className="text-2xl font-bold text-red-500">{noShow}</p>
          <p className="text-xs text-slate-600 mt-0.5">🔴 No Show</p>
        </div>
      </div>
      ...
      <div className="glass-card overflow-hidden">
        <div className="p-4 border-b border-gray-100 dark:border-white/10 flex items-center gap-2">
          ...
          <h2 className="text-sm font-semibold text-slate-950 dark:text-white">Today's Lecturer Roster</h2>
        </div>
              ...
              <tr key={l.id}>
                <td>
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-medium text-slate-950 dark:text-white">{l.name}</span>
                  </div>
                </td>
                <td>{l.scheduledClass}</td>
                <td className="text-xs font-mono">{l.room}</td>
                <td className="font-mono text-xs">{fmt(l.expectedStart)}</td>
                <td className={`font-mono text-xs ${l.actualCheckIn ? 'text-slate-800 dark:text-gray-300' : 'text-slate-600 dark:text-slate-400'}`}>
                  {fmt(l.actualCheckIn)}
                </td>
                ...
                <td>
                  <button
                    ...
                    className="text-xs font-medium px-2.5 py-1.5 rounded-lg bg-gray-100 dark:bg-white/5 text-slate-600 dark:text-slate-400 hover:bg-blue-50 hover:text-blue-600 dark:hover:bg-blue-500/10 dark:hover:text-blue-400 transition-colors cursor-pointer whitespace-nowrap flex items-center gap-1"
                  >
      ...
      <div>
        ...
        {showAudit && (
          <div className="glass-card overflow-hidden">
            {auditLog.length === 0 ? (
              <div className="p-8 text-center text-sm text-slate-600 dark:text-slate-400">No forced check-ins recorded yet.</div>
            ) : (
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-white/5">
                  {auditLog.map((entry) => (
                    <tr key={entry.id} className="hover:bg-gray-50 dark:hover:bg-white/3">
                      <td className="px-4 py-3 font-medium text-slate-950 dark:text-white">{entry.lecturerName}</td>
                      <td className="px-4 py-3 text-slate-600 dark:text-slate-400">{entry.course}</td>
                      <td className="px-4 py-3 text-xs text-slate-600">{entry.performedBy}</td>
                      ...
                      <td className="px-4 py-3 text-xs text-slate-600 dark:text-slate-400 max-w-[200px]">{entry.reason}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      {/* Info note */}
      <div className="flex items-start gap-2 text-xs text-slate-600 dark:text-slate-400 p-3 rounded-xl bg-gray-50 dark:bg-white/3">
