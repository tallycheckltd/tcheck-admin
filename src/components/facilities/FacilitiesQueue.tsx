...
const PRESET_META: Record<FacilityTicketPreset, { label: string; icon: typeof Wrench }> = {
  ...
  AV_ISSUE: { label: 'Projector / sound', icon: Tv },
  CATERING: { label: 'Refreshments / food', icon: Coffee },
  WIFI_INTERNET: { label: 'Wi-Fi / internet', icon: Wifi },
  SAFETY_MEDICAL: { label: 'Safety or medical', icon: ShieldAlert },
  OTHER: { label: 'Other', icon: HelpCircle },
};

/** FIXED (same class of bug as the mobile clients' preset decode): a plain `PRESET_META[x]`
 * index throws at render if `x` is ever a presetType this bundle predates — `presetType` is a
 * server-authoritative string, not something TypeScript's union actually constrains at runtime.
 * Falls back to the OTHER entry instead of crashing the whole ticket list/detail pane. */
function presetMeta(preset: string): { label: string; icon: typeof Wrench } {
  return PRESET_META[preset as FacilityTicketPreset] ?? PRESET_META.OTHER;
}
...
/** Appends `?cohortId=`/`&cohortId=` — /facility-tickets accepts it optionally
 * (facilityTicket.controller.ts) to narrow the whole queue to one particular programme. */
...
/**
 * The full stats-cards + search + status-tabs + list/detail triage queue — originally the admin
 * Facilities Queue page's body. Extracted so a CEM/Head of CEM's Facilities page (pooled across
 * every programme, or narrowed to one via `cohortId`) can feel exactly like the admin one instead
 * of the old compact accordion summary, per explicit request. One implementation shared by
 * `FacilitiesQueuePage` (admin, no cohortId), `CemFacilitiesPage` (CEM, pooled) and
 * `StaffPanelsGrid`'s facilities-only standalone page (CEM, one programme via `cohortId`) — so the
 * three surfaces can't drift apart. `MANAGE_FACILITY_TICKETS`-gated actions (reply/acknowledge/
...
export function FacilitiesQueue({ cohortId }: { cohortId?: string }) {
  ...
  const filtered = useMemo(() => {
    ...
    return list.filter((t) =>
      presetMeta(t.presetType).label.toLowerCase().includes(q) ||
      ...
      <div className="flex gap-6 h-[calc(100vh-22rem)]">
        <div className="w-96 flex flex-col">
          ...
          <div className="glass-card flex-1 overflow-y-auto p-2 space-y-1">
            {filtered.map((t) => {
              const u = urgency(t);
              const meta = presetMeta(t.presetType);
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex items-center gap-3">
                    <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-amber-500 to-orange-600 flex items-center justify-center text-white flex-shrink-0">
                      {(() => { const Icon = presetMeta(selected.presetType).icon; return <Icon size={22} />; })()}
                    </div>
                    <div>
                      <p className="text-base font-semibold text-slate-950 dark:text-white">
                        {presetMeta(selected.presetType).label}
                      </p>
